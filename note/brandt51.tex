...
\begin{corollary}[the spectrum of $\df$ below $3$]\label{cor:spectrum}
Assume Theorem~\ref{thm:lpr}. The set of values taken by $\df$ below $3$ on finite maximal
triangle-free graphs is exactly
\[
\Bigl\{\,3-\tfrac1k \;:\; k\in\mathbb Z_{\ge1}\Bigr\},
\]
a discrete set whose only accumulation point is $3$.
\end{corollary}

\begin{proof}
If $G$ is maximal triangle-free with $\df(G)<3$, Theorem~\ref{thm:main} makes $G$ a
nonempty blow-up of some $\Gamma_i$ or $\Upsilon_i^{\mu\nu}$, and $\df$ is a blow-up
invariant (Lemma~\ref{lem:blowup}), so by \eqref{eq:dfandrasfai} and
Theorem~\ref{thm:vega},
\[
\df(G)=3-\tfrac1i
\qquad\text{or}\qquad
\df(G)=3-\tfrac1{D_{\mu\nu}},
\]
and $D_{\mu\nu}=9i-6-\mu-\nu$ is a positive integer. Hence every value below $3$ has the
form $3-1/k$ with $k\in\mathbb Z_{\ge1}$. Conversely, $\Gamma_k$ is maximal triangle-free
and attains $3-1/k$ for every $k\ge1$ (for $k=1$, $\Gamma_1=K_2$ with $\df=2$). Thus the
Vega values form a subset of the Andr\'asfai values, and the spectrum is exactly as
stated.
\end{proof}

By contrast, the value $3$ itself is attained -- for instance by the graph $W$ of
Section~\ref{sec:sharp}, which is moreover not regularizable -- so the exclusion of
values applies only below the threshold.
...
The mathematics in this note was produced with substantial assistance from AI systems:
research sessions run with OpenAI's GPT~5.6 Sol Pro, directed by the author, with
independent verification and review by Anthropic's Claude Fable~5. Subsequently, every
step was verified independently of the originating sessions: Lemma~\ref{lem:duality} by
hand; the statements of Definition~\ref{def:dk} and Theorem~\ref{thm:lpr} against the
posted text of \cite{LPR}; Theorem~\ref{thm:vega} and Section~\ref{sec:sharp} by machine,
using the standalone scripts distributed with this note. A further independent AI-assisted
review identified the prior appearance of the Vega weights in \cite{BT,BP,LPR2021} and
supplied the shorter argument of Section~\ref{sec:short}; this version incorporates both
corrections. The author takes full responsibility for the content.
